...
%% Método VNS
 while cond==0
[b]=K_opt(a,nl2,k);                             % função k-opt
[dist]=distancia(b,nl2);                        % cálculo da distância total da rota
    if cont>3000 & cont<5001                    % condição para a aplicação do 3-opt
        k=3;
    else if cont>5000 & cont<6101               % condição para a aplicação do 4-opt
            k=5;
        else if cont>6100 & cont<6501           % condição para o retorno ao 2-opt
                k=7;
            else if cont>6500
                cont=1;
                k=2;
    ...
    if parada>200000                            % critério de parada
        cond=1;
    end
